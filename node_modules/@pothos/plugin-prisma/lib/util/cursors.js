...
_export(exports, {
    get cursorFormatter () {
        return cursorFormatter;
    },
    get formatCursorChunk () {
        return formatCursorChunk;
    },
    get formatPrismaCursor () {
        return formatPrismaCursor;
    },
    get getCursorFormatter () {
        return getCursorFormatter;
    },
    get getCursorParser () {
        return getCursorParser;
    },
    get getDefaultIDParser () {
        return getDefaultIDParser;
    },
    get getDefaultIDSerializer () {
        return getDefaultIDSerializer;
    },
    get parseCompositeCursor () {
        return parseCompositeCursor;
    },
    get parseID () {
        return parseID;
    },
    get parsePrismaCursor () {
        return parsePrismaCursor;
    },
    get prismaCursorConnectionQuery () {
        return prismaCursorConnectionQuery;
    },
    get resolvePrismaCursorConnection () {
        return resolvePrismaCursorConnection;
    },
    get serializeID () {
        return serializeID;
    },
    get wrapConnectionResult () {
        return wrapConnectionResult;
    }
});
const _core = require("@pothos/core");
const _datamodel = require("./datamodel.js");
const _usage = require("./usage.js");
...
function formatCursorChunk(value) {
    ...
    switch(typeof value){
        ...
        case 'bigint':
            return `I:${value}`;
        default:
            throw new _core.PothosValidationError(`Unsupported cursor type ${typeof value}`);
    }
}
function formatPrismaCursor(record, fields) {
...
function parsePrismaCursor(cursor) {
    if (typeof cursor !== 'string') {
        throw new _core.PothosValidationError('Cursor must be a string');
    }
    try {
        ...
        switch(type){
            ...
            case 'I':
                return BigInt(value);
            default:
                throw new _core.PothosValidationError(`Invalid cursor type ${type}`);
        }
    } catch  {
        throw new _core.PothosValidationError(`Invalid cursor: ${cursor}`);
    }
}
...
function getDefaultIDSerializer(modelName, fieldName, builder) {
    ...
    if (((_ref = (_model_primaryKey = model.primaryKey) === null || _model_primaryKey === void 0 ? void 0 : _model_primaryKey.name) !== null && _ref !== void 0 ? _ref : (_model_primaryKey1 = model.primaryKey) === null || _model_primaryKey1 === void 0 ? void 0 : _model_primaryKey1.fields.join('_')) === fieldName) {
    ...
    throw new _core.PothosValidationError(`Unable to find ${fieldName} for model ${modelName}`);
}
function getDefaultIDParser(modelName, fieldName, builder) {
    var _ref;
    var _model_primaryKey, _model_primaryKey1;
    if (!fieldName) {
        throw new _core.PothosValidationError('Missing field name');
    }
    const model = (0, _datamodel.getModel)(modelName, builder);
    const field = model.fields.find((f)=>f.name === fieldName);
    if (field) {
    ...
    return (id)=>{
        ...
        if (!Array.isArray(parts)) {
            throw new _core.PothosValidationError(`Invalid id received for ${fieldName} of ${modelName}`);
        }
        const result = {};
...
function parseCompositeCursor(fields) {
    return (cursor)=>{
        ...
        if (!Array.isArray(parsed)) {
            throw new _core.PothosValidationError(`Expected compound cursor to contain an array, but got ${parsed}`);
        }
        const record = {};
...
function prismaCursorConnectionQuery({ args, ctx, maxSize = DEFAULT_MAX_SIZE, defaultSize = DEFAULT_SIZE, parseCursor }) {
    ...
    if (before && after) {
        throw new _core.PothosValidationError('Arguments "before" and "after" are not supported at the same time');
    }
    if (before != null && first != null) {
        throw new _core.PothosValidationError('Arguments "before" and "first" are not supported at the same time');
    }
    if (after != null && last != null) {
        throw new _core.PothosValidationError('Arguments "after" and "last" are not supported at the same time');
    }
...
function wrapConnectionResult(parent, results, args, take, cursor, totalCount, resolveNode) {
    ...
    connection.pageInfo.startCursor = (_ref = (_edges_ = edges[0]) === null || _edges_ === void 0 ? void 0 : _edges_.cursor) !== null && _ref !== void 0 ? _ref : null;
    connection.pageInfo.endCursor = (_ref1 = (_edges_1 = edges[edges.length - 1]) === null || _edges_1 === void 0 ? void 0 : _edges_1.cursor) !== null && _ref1 !== void 0 ? _ref1 : null;
    return connection;
}
async function resolvePrismaCursorConnection(options, cursor, resolve) {
    const query = prismaCursorConnectionQuery(options);
    const results = await resolve((0, _usage.extendWithUsage)(options.query, query));
    if (!results) {
        return results;
    }
...
function getCursorFormatter(name, builder, cursor) {
    ...
    const primaryKey = (_ref = (_modelData_primaryKey = modelData.primaryKey) === null || _modelData_primaryKey === void 0 ? void 0 : _modelData_primaryKey.name) !== null && _ref !== void 0 ? _ref : (_modelData_primaryKey1 = modelData.primaryKey) === null || _modelData_primaryKey1 === void 0 ? void 0 : _modelData_primaryKey1.fields.join('_');
    ...
    return cursorFormatter((_ref1 = uniqueIndex === null || uniqueIndex === void 0 ? void 0 : uniqueIndex.fields) !== null && _ref1 !== void 0 ? _ref1 : cursor);
}
