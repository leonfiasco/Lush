...
(0, _generatorhelper.generatorHandler)({
    ...
    onGenerate: async (options)=>{
        var _ref, _config_clientOutput, _ref1;
        ...
        const outputLocation = (_ref1 = (_options_generator_output = options.generator.output) === null || _options_generator_output === void 0 ? void 0 : _options_generator_output.value) !== null && _ref1 !== void 0 ? _ref1 : defaultOutput;
...
async function generateOutput(dmmf, prismaTypes, prismaLocation, outputLocation, config, datamodel) {
    ...
    const prismaImportStatement = `import type { ${[
        'Prisma',
        ...modelNames
    ].join(', ')} } from ${stringLiteral(prismaLocation)};`;
    const dmmfImportStatement = `import type { PothosPrismaDatamodel } from ${stringLiteral((_config_pluginPath = config.pluginPath) !== null && _config_pluginPath !== void 0 ? _config_pluginPath : '@pothos/plugin-prisma')};`;
    const trimmedDatamodel = trimDmmf(dmmf, config.documentation === 'true');
    const dmmfExport = outputLocation.endsWith('.d.ts') ? 'export function getDatamodel(): PothosPrismaDatamodel;' : `export function getDatamodel(): PothosPrismaDatamodel { return JSON.parse(${JSON.stringify(JSON.stringify(trimmedDatamodel))}); }`;
    const statements = config.generateDatamodel !== 'false' ? [
    ...
    (0, _nodefs.mkdirSync)((0, _nodepath.dirname)(outputLocation), {
        recursive: true
    });
    (0, _nodefs.writeFileSync)(outputLocation, `/* eslint-disable */\n${statements.join('\n')}`);
...
function buildTypes(dmmf, config) {
    ...
    const modelTypes = dmmf.datamodel.models.map((model)=>{
        var _dmmf_schema_inputObjectTypes_prisma, _dmmf_schema_inputObjectTypes_prisma1;
        ...
        const updateInputUnavailable = !((_dmmf_schema_inputObjectTypes_prisma1 = dmmf.schema.inputObjectTypes.prisma) === null || _dmmf_schema_inputObjectTypes_prisma1 === void 0 ? void 0 : _dmmf_schema_inputObjectTypes_prisma1.some((input)=>input.name === `${model.name}UpdateInput`));
        const relationEntries = relations.map((field)=>{
            const typeName = field.type;
            const shape = field.isList ? `${typeName}[]` : field.isRequired ? typeName : `${typeName} | null`;
            return [
                `            ${field.name}: {`,
                `                Shape: ${shape};`,
        ...
        return [
            ...
            relationEntries.length > 0 ? `        Relations: {\n${relationEntries.join('\n')}\n        };` : '        Relations: {};',
            '    };'
        ].join('\n');
    });
    return [
